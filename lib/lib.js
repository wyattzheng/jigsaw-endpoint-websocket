...
class WSEndpoint extends EventEmitter{
	...
	_start(){
		...
		this.heartbeat_checker=setInterval(this._checkAlive.bind(this),1000);
	}
	close(){
		if(this.state=="close")return;

		clearInterval(this.heartbeat_checker);
		this.server.close();
		this.state="close";
	}
	_checkAlive(){
		for(let [conn,info] of this._conns.entries())
			if(info.timeout--<0)
				conn.close();
	}
	...
	_onWSConnection(conn,req){
		...
		try{
			...
			let conninfos=Array.from(this._conns.values());

			assert(path_parsed.dir=="/","please provide a valid address");
			assert(path_parsed.name.length > 0,"please provide a valid address");
			assert(conninfos.length < this._connlimit,"the amount of connections reach its max limit.")
			...
			this._conns.set(conn,{exists:true,jgname,ports:new Set(),jigsaw:jg,timeout:40});


			this.emit("enter",jgname);

		}catch(err){
			this._sendResponse(conn,"error",{err:err.stack});
			conn.close();
			return;
		}
	...
	async _handleMessage(pk,conn){
		...
		switch(pk.type){
			...
			case "unport":{
				...
				if(!conninfo.ports.has(pk.portname))
					throw new Error("this port hasn't binded");


				conninfo.ports.delete(pk.portname);
				//conninfo.jigsaw.unport(pk.portname);
				delete conninfo.jigsaw.producer.ports[pk.portname];
